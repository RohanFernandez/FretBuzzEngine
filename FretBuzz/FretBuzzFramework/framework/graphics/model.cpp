...
namespace ns_fretBuzz
{
	...
	void Model::loadTextureOfType(std::vector<Mesh::MeshTexture>& a_vectTextures, aiTextureType a_TextureType, aiMaterial& a_Material)
	{
		...
		if (l_iCurrentTexTypeCount > 0)
		{
			...
			Texture* l_pTexture = ResourceManager::getResource<Texture>(l_strTexturePath);
			if (l_pTexture == nullptr)
			{
				Texture l_texture(l_strTexturePath);
				ResourceManager::s_addResource<Texture>(l_strTexturePath, l_texture);
				l_pTexture = ResourceManager::getResource<Texture>(l_strTexturePath);
			}
	...
	void Model::addToGameObject(GameObject& a_GameObject, Model& a_Model)
	{
	...
	void Model::addMeshToGameObject(GameObject& a_GameObject, Node& a_Node)
	{
		...
		for (size_t l_iChildIndex = 0; l_iChildIndex < l_iChildCount; l_iChildIndex++)
		{
			...
			GameObject* l_pChildGameObject = GameObject::instantiate(a_GameObject, l_CurrentNode.m_strName);
