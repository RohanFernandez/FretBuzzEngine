...
namespace FRETBUZZ
{
	...
	{
		m_pSpriteRenderer = m_GameObject.getComponent<SpriteRenderer>(COMPONENT_TYPE::SPRITE_RENDERER);
		if (m_pSpriteRenderer == nullptr)
		{
			m_pSpriteRenderer = m_GameObject.addComponent<SpriteRenderer>(nullptr);
		}
	...
	void SpriteAnimationController::playStateID(std::string a_strAnimationID)
	{
		...
		SpriteGroup* l_pCurrentSpriteGroup = ResourceManager::getResource<SpriteGroup>(m_pAnimState->getSpriteGroupID());
